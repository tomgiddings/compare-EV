const Footer: React.FC = () => {
    return (
        <footer className="footer mt-5 mb-5 p-5 md:p-0 z-10 text-sm">
        <div className="container mx-auto flex">
            <a href="https://github.com/tomgiddings/evee" className="flex gap-x-1 items-center">
                <figure>
                    <svg 
                        xmlns="http://www.w3.org/2000/svg"
                        xmlnsXlink="http://www.w3.org/1999/xlink"
                        width="11.289mm" height="10.936mm">
                        <image x="0px" y="0px" width="32px" height="31px"  xlinkHref="data:img/png;base64," />
                    </svg>
                </figure>
                View the code and contribute to CompareEV on GitHub!
            </a>
        </div>
        </footer>
    );
}

export default Footer;
